...
int set_opt(jint nBits, jchar nEvent, jint nStop, jint speed) {
    LOGE("set_opt:nBits=%d,nEvent=%c,nStop=%d,speed=%d", nBits, nEvent, nStop, speed);
...
//        cfmakeraw(&cfg);
        //设置波特率
        int nRt0 = cfsetispeed(&cfg, speed);
        int nRt1 = cfsetospeed(&cfg, speed);
        LOGE("nRt0=%d,nRt1=%d", nRt0, nRt1);

        if (tcsetattr(fd, TCSANOW, &cfg)) {
            LOGE("tcsetattr() failed2");
            close(fd);
            /* TODO: throw an exception */
            return NULL;
        }
        ...
        switch (databits) //设置数据位数
        {
            case 7:
                LOGE("设置数据位数==7");
                cfg.c_cflag = (cfg.c_cflag & ~CSIZE) | CS7;
                break;
            case 8:
                LOGE("设置数据位数==8");
...
Java_com_google_serial_SerialPort_close(JNIEnv *env, jobject thiz) {
    ...
    jfieldID mFdID = (*env)->GetFieldID(env, SerialPortClass, "descriptor", "Ljava/io/FileDescriptor;");
